...
-   **breadcrumbs** are now able to retrieve a route's data from the `RouteDataRegistry` when it is not provided with the route itself. Replaced all `route.children` with `getChildren()` method for lazy loading

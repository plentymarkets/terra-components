...
export class TerraBreadcrumbsService {
  ...
  public updateBreadcrumbNameByUrl(url: string, name?: string): void {
    ...
    if (
      ...
      !isNullOrUndefined(route.data) &&
      !isNullOrUndefined(breadcrumb)
    ) {
  ...
  private _handleBreadcrumb(
    route: Route,
    url: string,
    cleanEventUrl: string,
    urlPartsCount: number
  ): void {
    ...
    if (isNullOrUndefined(breadcrumb)) {
      ...
      if (UrlHelper.getCleanUrl(cleanEventUrl) === UrlHelper.getCleanUrl(url)) {
        breadcrumb = new TerraBreadcrumb(
          label,
          parentBreadcrumb,
          url,
  ...
  private _updateBreadcrumbVisibilities(
    ...
    breadcrumbContainer.breadcrumbList.forEach((bc: TerraBreadcrumb) => {
      bc.isHidden =
        bc.parent !== parentBreadcrumb || (!isNullOrUndefined(bc.parent) && bc.parent.isHidden);
    });
  ...
  private _findActivatedRouteSnapshot(
  ...
  private _removeBreadcrumbsByParent(
    ...
    let currentContainerIndex: number = this._containers.indexOf(container);
    let nextContainer: TerraBreadcrumbContainer = this._containers[currentContainerIndex + 1];
    let filteredBreadcrumbList: Array<TerraBreadcrumb> = container.breadcrumbList.filter(
      (bc: TerraBreadcrumb) => {

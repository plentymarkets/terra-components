...
export class TerraPagerComponent implements OnInit, OnDestroy {
  ...
  @Output()
  public outputDoPaging: EventEmitter<TerraPagerInterface<any>> = new EventEmitter<
    TerraPagerInterface<any>
  >();

  @Language()
  public _lang: string;

  private _pagingClicks: Subject<TerraPagerInterface<any>> = new Subject<
    TerraPagerInterface<any>
  >();
  ...
  public onToPage(event: any, pageNumber: number): void {
    ...
    this.inputPagingData.page = Math.max(
      1,
      Math.min(this.inputPagingData.lastPageNumber, pageNumber)
    );

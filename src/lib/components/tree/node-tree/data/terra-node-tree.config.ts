...
export class TerraNodeTreeConfig<D> {
   ...
   */
  public addNode(
    nodeToAdd: TerraNodeInterface<D>,
    ...
    if (isNullOrUndefined(alreadyAddedNode)) {
      ...
      if (!isNullOrUndefined(nodeToAdd.children)) {
        TerraNodeTreeHelper.recursiveSetParentAndDefaultVisibility<D>(
          nodeToAdd.children,
          nodeToAdd
        );
      }
   ...
   * @param openParents Optional. Open all parents.
   */
  public addChildToNodeById(
    parentId: string | number,
    node: TerraNodeInterface<D>,
    openParents?: boolean
  ): void {
    let foundNode: TerraNodeInterface<D> = TerraNodeTreeHelper.recursiveFindNodeById<D>(
      this.list,
      parentId
    );
   ...
   */
  public toggleVisiblityForAllChildren(
    nodeList: Array<TerraNodeInterface<D>>,
    isVisible: boolean
  ): void {
    nodeList.forEach((node: TerraNodeInterface<D>) => {
   ...
   * @param isVisible Toggle visibility.
   */
  public toggleVisibilityForAllParents(
    parentNode: TerraNodeInterface<D>,
    isVisible: boolean
  ): void {

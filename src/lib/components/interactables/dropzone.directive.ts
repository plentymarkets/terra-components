...
export class TerraDropzoneDirective implements OnInit, OnChanges {
  ...
  constructor(private _el: ElementRef) {
    this._init();
    console.warn(
      'TerraDropzoneDirective is deprecated. Please use another DnD library e.g. Angular Material CDK.'
    );
  }
  ...
  public ngOnInit(): void {
    if (this.onDropActivate.observers.length > 0) {
      console.warn(
        '`terra-dropzone-onDropActivate` is deprecated. Please use `dropActivate` instead.'
      );
    }

    if (this.onDropDeactivate.observers.length > 0) {
      console.warn(
        '`terra-dropzone-onDropDeactivate` is deprecated. Please use `dropDeactivate` instead.'
      );
    }
  ...
  private _initConfigObject(createDropEvent: (event: DropEvent) => DropEvent): any {
    return {
      ...
      ondropactivate: (event: DropEvent): void => {
        this.onDropActivate.emit(createDropEvent(event));
        this.dropActivate.emit(createDropEvent(event));
      },
      ondropdeactivate: (event: DropEvent): void => {
        this.onDropDeactivate.emit(createDropEvent(event));
        this.dropDeactivate.emit(createDropEvent(event));
      },
      ondragenter: (event: DropEvent): void => {
        this.onDragEnter.emit(createDropEvent(event));
        this.dragEnter.emit(createDropEvent(event));
      },
      ondragleave: (event: DropEvent): void => {
        this.onDragLeave.emit(createDropEvent(event));
        this.dragLeave.emit(createDropEvent(event));
      },
      ondropmove: (event: DropEvent): void => {
        this.onDropMove.emit(createDropEvent(event));
        this.dropMove.emit(createDropEvent(event));
      },
      ondrop: (event: DropEvent): void => {
        this.onDrop.emit(createDropEvent(event));
        this.drop.emit(createDropEvent(event));
      }
    };
  }
}

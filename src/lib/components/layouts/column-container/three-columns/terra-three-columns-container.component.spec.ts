...
describe('Component: TerraThreeColumnsContainerComponent', () => {
  ...
  beforeEach(async(() => {
    TestBed.configureTestingModule({
      declarations: [TerraThreeColumnsContainerComponent]
    }).compileComponents();
  }));
  ...
  beforeEach(() => {
    ...
    columns = fixture.debugElement
      .query(By.css('.row'))
      .children.map((de: DebugElement) => de.nativeElement);
  ...
  it('should initialize all columns with a width of 12 on extra small screens', () => {
    expect(
      columns.every((column: HTMLDivElement) => column.classList.contains('col-12'))
    ).toBeTruthy();
  });

  it('should initialize all columns with a width of 4 on medium screens', () => {
    expect(
      columns.every((column: HTMLDivElement) => column.classList.contains('col-md-4'))
    ).toBeTruthy();
  });

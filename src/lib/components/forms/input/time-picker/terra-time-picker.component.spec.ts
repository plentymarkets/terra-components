...
describe('TerraTimePickerComponent:', () => {
    ...
    beforeEach(() => {
        TestBed.configureTestingModule({
            imports: [ReactiveFormsModule, TranslationModule.forRoot({})],
    ...
    it('should initialize possible values during construction', () => {
        ...
        expect(
            component.valuesHours.every((val: TerraSelectBoxValueInterface, index: number) => val.value === index)
        ).toBe(true);
        expect(component.valuesMinutes.length).toBe(60);
        expect(
            component.valuesMinutes.every((val: TerraSelectBoxValueInterface, index: number) => val.value === index)
        ).toBe(true);
    });
    ...
    it('should set the hours when the user selects a different hours', () => {
        const hourSelect: TerraSelectBoxComponent = fixture.debugElement.queryAll(
            By.directive(TerraSelectBoxComponent)
        )[0].componentInstance;
    ...
    it('should set the minutes when the user selects a different minute', () => {
        const minuteSelect: TerraSelectBoxComponent = fixture.debugElement.queryAll(
            By.directive(TerraSelectBoxComponent)
        )[1].componentInstance;

...
export class CheckboxGroupComponent implements ControlValueAccessor {
  ...
  public _onMultiCheckboxChanged(checkboxValues: Array<TerraMultiCheckBoxValueInterface>): void {
    ...
    if (this._values.length === 0) {
      ...
      this._values.sort((a: any, b: any): number => {
        let apos: number = this._multiCheckboxValues.findIndex(
          (checkbox: TerraMultiCheckBoxValueInterface) => {
            return checkbox.value === a;
          }
        );
        let bpos: number = this._multiCheckboxValues.findIndex(
          (checkbox: TerraMultiCheckBoxValueInterface) => {
  ...
  private _updateMultiCheckboxValues(): void {
    this._multiCheckboxValues = this.checkboxValues.map(
      (checkbox: { caption: string; value: any }) => {
        return {

...
export class TerraFormFieldHelper {
    ...
    if (!isNullOrUndefined(formFields)) {
      Object.keys(formFields).forEach((formFieldProperty: string) => {
        ...
        if (
          ...
          Reflect.hasMetadata(
            TERRA_FORM_PROPERTY_METADATA_KEY,
            formModel[formFieldProperty].constructor
          )
        ) {
  ...
  public static injectOption(
    ...
    Object.keys(formFields).forEach((key: string) => {
      ...
      if (!isNullOrUndefined(formFields[key].children)) {
        formFields[key].children = this.injectOption(
          formFields[key].children,
          type,
          optionKey,
          optionValue
        );
  ...
  public static isLegacyFormFields(
    ...
    if (isArray(formFields)) {
      ...
      formFields.forEach((field: TerraFormFieldBase<any>) => {
        let transformedField: {
          key: string;
          field: TerraFormFieldInterface;
        } = this._transformLegacyFormField(field);
  ...
  public static parseDefaultValue(
    formField: TerraFormFieldInterface,
   ...
   * @param formFields
   */
  public static parseDefaultValues(
    formFields: TerraKeyValueInterface<TerraFormFieldInterface>
  ): any {
    if (isNullOrUndefined(formFields)) {
      return;
    }

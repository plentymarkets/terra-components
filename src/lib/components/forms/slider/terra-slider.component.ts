...
export class TerraSliderComponent implements OnInit, OnChanges, ControlValueAccessor {
  ...
  public ngOnInit(): void {
    ...
    if (isNullOrUndefined(this.inputPrecision)) {
      if (this.inputInterval > 0) {
        ...
        this.inputPrecision =
          5 - Math.max(('' + this.inputMin).length, ('' + this.inputMax).length);
      }
    }
  ...
  public _getTicks(): Array<TerraSliderTick> {
    ...
    for (let i: number = 1; i < numberOfTicks; i++) {
      ...
      ticks.push(
        new TerraSliderTick(positionInPercent, this.calculateValueFromPercent(positionInPercent))
      );
    }
  ...
  public get _grid(): GridOptions {
    if (this.inputInterval > 0) {
      return {
        x:
          this._element.nativeElement.getBoundingClientRect().width / this.calculateNumberOfSteps(),
        y: 0
      };
    }
  ...
  private moveToPosition(position: number): void {
    if (!this.inputIsDisabled) {
      let sliderRect:
        | any
        | ClientRect = this._sliderBarElement.nativeElement.getBoundingClientRect();

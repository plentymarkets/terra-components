...
export class TerraSimpleTableComponent<D> implements OnChanges {
  ...
  @Output()
  public outputRowCheckBoxChanged: EventEmitter<
    TerraSimpleTableRowInterface<D>
  > = new EventEmitter();
  ...
  @Output()
  public outputHighlightedRowChange: EventEmitter<
    TerraSimpleTableRowInterface<D>
  > = new EventEmitter();

  @Output()
  public outputSelectedRowsChange: EventEmitter<
    Array<TerraSimpleTableRowInterface<D>>
  > = new EventEmitter();
  ...
  public get selectedRowList(): Array<TerraSimpleTableRowInterface<D>> {
    return this.inputRowList.filter(
      (row: TerraSimpleTableRowInterface<D>) => row.selected === true
    );
  }
      ...
      if (highlightIndex >= 0 && highlightIndex < this.inputRowList.length) {
        ...
        if (viewport.bottom < activeRowPosition.bottom) {
          this.scrollContainer.nativeElement.scrollTop +=
            activeRowPosition.bottom - viewport.bottom;

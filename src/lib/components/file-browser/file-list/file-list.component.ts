...
export class TerraFileListComponent implements OnInit, AfterViewInit, OnChanges, OnDestroy {
  ...
  public ngOnInit(): void {
    ...
    this._dragSubscription = this._fileBrowserService.isDragActive.subscribe(
  ...
  public onSelectionChange(rows: Array<TerraSimpleTableRowInterface<TerraStorageObject>>): void {
    this._selectedStorageObjects = rows.map(
      (row: TerraSimpleTableRowInterface<TerraStorageObject>) => {
        return row.value;
      }
    );
  }
  ...
  public _deleteObjects(): void {
    ...
    this.activeStorageService.deleteFiles(keyList).subscribe(() => {
      ...
      if (
        !isNullOrUndefined(this.imagePreviewObject) &&
        keyList.indexOf(this.imagePreviewObject.key) >= 0
      ) {
        this.imagePreviewObject = null;
        this.hideImagePreview.emit();
      }
  ...
  private _createTableRow(
    ...
    if (
      this.activeStorageService instanceof TerraBasePrivateStorageService &&
      storageObject.isFile
    ) {
      ...
      buttonList: [
        {
          icon: 'icon-download',
          clickFunction: (event: Event): void => {
            (<TerraBasePrivateStorageService>this.activeStorageService).downloadFile(
              storageObject.key
            );
            event.stopPropagation();
          },
          tooltipText: this._translationService.translate(
            this._translationPrefix + '.downloadFile'
          ),
          tooltipPlacement: 'left'
        }
      ]
  ...
  private _isAllowed(filename: string): boolean {
    ...
    return (
      ...
      this._parentFileBrowser.inputAllowedExtensions.indexOf(
        PathHelper.extName(filename).toUpperCase()
      ) >= 0 ||
  ...
  private _uploadFiles(fileList: FileList | Array<File>): void {
    const queueLength: number = !this._progress
      ? 0
      : this._progress.filesTotal - this._progress.filesUploaded;
    if (fileList.length + queueLength > MAX_UPLOAD_COUNT) {
      this._alertService.error(
        this._translationService.translate('terraFileBrowser.error.tooManyFiles', {

...
export class TerraUploadQueue {
  ...
  public status: BehaviorSubject<TerraUploadProgress> = new BehaviorSubject<TerraUploadProgress>(
    null
  );
  ...
  constructor(
    private _uploadUrl: string | UploadQueueUrlFactory,
    private _uploadMethod: 'GET' | 'POST' | 'DELETE' | 'PUT' = 'POST'
  ) {
    this.progress = new Observable(
      (observer: Observer<number>): Function => {
  ...
  public startUpload(): Promise<void> {
    ...
    this.inProgress = this._uploadAllItems().then(() => {
  ...
  private _uploadItem(item: TerraUploadItem): Promise<void> {
    return new Promise((resolve: (resp: void) => void, reject: (err: any) => void): void => {
      ...
      xhr.open(this._uploadMethod, this._getUploadUrl(item.pathname), true);
  ...
  private _onProgress(): void {
    let filesUploaded: Array<TerraUploadItem> = this._items.filter(
      (item: TerraUploadItem) => item.uploaded
    );

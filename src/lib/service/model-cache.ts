...
export class ModelCache {
   ...
   * @param dataId
   */
  protected handleLocalDataModelGet(
   ...
   * @param dataId
   */
  protected handleLocalDataModelPost(
   ...
   * @param dataId
   */
  protected handleLocalDataModelPut(
   ...
   * @param dataId
   */
  protected handleLocalDataModelDelete(
    deleteRequest$: Observable<void>,
    dataId: number | string
  ): Observable<void> {
    return deleteRequest$.pipe(
      tap(() => {
        Object.keys(this.dataModel).forEach((comparisonId: string) => {
          let dataIndex: number = this.dataModel[comparisonId].findIndex(
            (data: any) => data.id === dataId
          );
          if (dataIndex >= 0) {

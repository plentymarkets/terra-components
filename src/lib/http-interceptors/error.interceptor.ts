...
export class ErrorInterceptor implements HttpInterceptor {
      ...
      catchError((error: HttpErrorResponse) => {
        ...
        if (error.status === 403) {
          ...
          if (error.error && error.error['error'] && error.error['error'][missingPermissionsKey]) {
            let errorMessage: string = this._translation.translate(
              'errorInterceptor.missingPermissions'
            );
  ...
  private _getMissingPermissionTranslations(
    ...
    return Object.values(missingPermissions).map(
      (missingPermission: { lang: string }) => missingPermission[lang]
    );
  }
}
